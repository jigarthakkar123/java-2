...
public class CarDao {
	...
	public static void updateCar(Car c)
	{
		try {
			Connection conn=UserUtil.create_conn();
			String sql="update tblcar set car_company=?,car_name=?,car_mileage=?,car_transmission=?,car_capacity=?,car_luggage_cap=?,car_fuel=?,car_ppd=? where cid=?";
			PreparedStatement pst=conn.prepareStatement(sql);
	...
	public static void deleteCar(int cid)
	{
		try {
			Connection conn=UserUtil.create_conn();
			String sql="delete from tblcar where cid=?";
			PreparedStatement pst=conn.prepareStatement(sql);
			pst.setInt(1, cid);
			pst.executeUpdate();
		} catch (Exception e) {
	...
	public static Car getCar(int cid)
	{
		...
		try {
			Connection conn=UserUtil.create_conn();
			String sql="select * from tblcar where cid=?";
			PreparedStatement pst=conn.prepareStatement(sql);
			pst.setInt(1, cid);
			ResultSet rs=pst.executeQuery();
			if(rs.next())
